...
async def auth(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if not user:
        user = await db_manager.create_user(email=user_info['email'], first_name=user_info['first_name'], last_name=user_info['last_name'], picture=user_info['picture'])
    ...
    access_jwt_token = await create_access_token(
        data=data
    )
    refresh_jwt_token = await create_refresh_token(
        data=data
    )
    ...
    response.set_cookie(key="access_token", httponly=True, value=f"Bearer {access_jwt_token}", secure=True, samesite='lax')
    response.set_cookie(key="refresh_token", httponly=True, value=f"Bearer {refresh_jwt_token}", secure=True, samesite='lax')

    ...
    async def create_subscription_plan(db: AsyncSession):
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response_standart_month.status_code == 201:
                ...
                db_manager.create_paypal_plan(plan_id=plan_standart_month["id"], status=plan_standart_month["status"], name=plan_standart_month["name"], period="month", plan="standart")

            if response_premium_month.status_code == 201:
                plan_premium_month = response_premium_month.json()
                db_manager.create_paypal_plan(plan_id=plan_premium_month["id"], status=plan_premium_month["status"], name=plan_premium_month["name"], period="month", plan="premium")

            if response_standart_year.status_code == 201:
                plan_standart_year = response_standart_year.json()
                db_manager.create_paypal_plan(plan_id=plan_standart_year["id"], status=plan_standart_year["status"], name=plan_standart_year["name"], period="year", plan="standart")

            if response_premium_year.status_code == 201:
                plan_premium_year = response_premium_year.json()
                db_manager.create_paypal_plan(plan_id=plan_premium_year["id"], status=plan_premium_year["status"], name=plan_premium_year["name"], period="year", plan="standart")
...
async def create_subscription(request: SubscriptionRequest, current_user: UserOrm = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    db_manager = DBManager(session=db)

    paypal_plan = await db_manager.get_paypal_plan(plan=request.plan, period=request.period)
...
async def cancel_subscription(reason: str, current_user: UserOrm = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    db_manager = DBManager(session=db)
...
async def suspend_subscription(reason: str, current_user: UserOrm = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    db_manager = DBManager(session=db)
...
async def handle_webhook(request: Request):
    ...
    if not await PaypalPayments.verify_webhook_signature(body=event_body, headers=headers):
        raise HTTPException(status_code=400, detail="Invalid webhook signature")
    
    event_type = event_body.get("event_type")
    
    if event_type == "BILLING.SUBSCRIPTION.CREATED":
        new_subscription = await subscribe(request.email, )

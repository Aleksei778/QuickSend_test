...
async def create_payment(subscription_id: int, current_user: UserOrm = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    # Находим подписку 
    stmt = select(SubscriptionOrm).where(SubscriptionOrm.id == subscription_id, SubscriptionOrm.user_id == current_user.id)
    ...
    return {"payment_id": new_payment.id, "amount": amount, "currency": "RUB", "status": "pending"}

@payment_router.post("/confirm_payment/{payment_id}")
async def confirm_payment(payment_id: int, current_user: UserOrm = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    stmt = select(PaymentOrm).join(SubscriptionOrm).where(

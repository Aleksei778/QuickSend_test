...
async def run_campaign2(
    ...
    attachment_paths = [prep_attachment["filename"] for prep_attachment in prep_attachments]
...
async def run_campaign1(files: List[UploadFile] = File(...), body: str = Form(...), current_user = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
# Получаем тело запроса в виде JSON-данных
    ...
    if (campaign_data.get('date') and campaign_data.get('time')):
        ...
        await mass_email_campaign(sender=sender_email, recipients=recipients, subject=subject, body_template=body_template, attachments=prep_attachments, sender_name=sender_name)
...
async def get_camps_stat(request: Request, current_user = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    db_manager = DBManager(session=db)
